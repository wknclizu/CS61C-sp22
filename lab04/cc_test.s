...
inc_arr_loop:
    ...
    # because we use t1 without condisering its initial value
    # when we return from helper_fn, the program would take t1 as garbage
    # when i change t1 into t0, the program gets wrong
    # which means when return from function, the value in Temporary Registers would change
    # And it is a good habit to preserve ra when calling a function
    jal helper_fn
    # FIXME Restore t0
    lw ra, 0(ra)
    lw t0, 4(sp)
